...
onready var playr_wniosek2 = get_node("../Player/Camera2D/CanvasLayer/HBoxContainer/33")
	...
	if Global.nowy_wniosek == true:
		...
		if event.is_action_pressed("LPM") and close == true :
			...
			if Global.okienko1 == false:
				...
				Global.okienko8 = true
				#zaliczony_wniosek
				Global.atrybut2 = true
				playr_wniosek.visible = false
				$AudioStreamPlayer.play()
				playr_wniosek2.visible = true
				check()
				$AnimationPlayer.play("wejscie")
			else: 
				check()
